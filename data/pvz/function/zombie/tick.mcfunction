...
execute as @e[tag=plot.root] if score @s generic.id = @n[tag=this.zombie] generic.id run tag @s add this.plot
...
execute unless entity @n[tag=this.plot] as @s[tag=!death] run function pvz:zombie/die
